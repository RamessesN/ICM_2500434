...
# Country codes to full country names mapping
COUNTRY_CODES = {
...
def save_country_stats(country_attacks: dict, output_file: str) -> None:
    df = pd.DataFrame(list(country_attacks.items()), columns=['Country', 'Attack Count'])
    df = df.sort_values('Attack Count', ascending=False)
    df.to_excel(output_file, index=False, engine='openpyxl')
    print(f"Country statistics saved: {output_file}")

def main() -> None:
    json_directory = "./data/json"  # Specify your directory containing JSON files
    output_excel = "./result/excel/output_network_crime_data.xlsx"
    country_stats_excel = "./result/excel/country_attack_statistics.xlsx"

    # Ensure output directories exist
    os.makedirs(os.path.dirname(output_excel), exist_ok=True)
    os.makedirs(os.path.dirname(country_stats_excel), exist_ok=True)
