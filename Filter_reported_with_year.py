...
def extract_successful_countries(json_data):
    ...
    if security_incident != "Near miss":
        return []
...
def process_json_files(json_dir):
    """
    遍历指定目录中的所有 JSON 文件，按年份统计每个国家成功的攻击次数。
    """
    year_country_count = {}  # 按年份和国家统计攻击次数
    for filename in os.listdir(json_dir):
        if filename.endswith(".json"):
            ...
            # 获取年份
            year = json_data.get("timeline", {}).get("incident", {}).get("year")
            if not year:
                continue  # 如果没有年份信息，跳过该文件
            ...
            for code in country_codes:
                if code != 'Unknown':
                    # 初始化年份和国家的统计
                    if year not in year_country_count:
                        year_country_count[year] = {}
                    if code in year_country_count[year]:
                        year_country_count[year][code] += 1
                    else:
                        year_country_count[year][code] = 1

    return year_country_count

def save_successful_countries(year_country_count, output_file):
    """
    将按年份和国家统计的攻击次数保存到 Excel 文件中。
    """
    if not year_country_count:
        print(f"No near miss attacks found. {output_file} not created.")
        return

    # 构建包含年份、国家代码、国家全名和攻击次数的列表
    data = []
    for year, country_count in year_country_count.items():
        for code, count in country_count.items():
            name = get_full_country_name(code)
            data.append({'Year': year, 'Country Code': code, 'Country': name, 'Attacks': count})

    # 转换为 DataFrame 并保存为 Excel
    df = pd.DataFrame(data)
    df = df.sort_values(by=['Year', 'Attacks'], ascending=[True, False])
    df.to_excel(output_file, index=False, engine='openpyxl')
    print(f"Near miss countries by year saved: {output_file}")
...
def main():
    ...
    nearMiss_stats_excel = "./result/excel/near_miss_countries_by_year.xlsx"  # 输出 Excel 文件路径

    # 确保输出目录存在
    os.makedirs(os.path.dirname(nearMiss_stats_excel), exist_ok=True)

    # 处理 JSON 文件，获取按年份和国家统计的攻击次数
    year_country_count = process_json_files(json_directory)

    # 保存结果到 Excel 文件
    save_successful_countries(year_country_count, nearMiss_stats_excel)
