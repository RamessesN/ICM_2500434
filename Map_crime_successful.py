...
def generate_heatmap(excel_path, shapefile_path, output_path):
    ...
    # 检查必要的列是否存在
    if 'Country' not in df_country_attacks.columns or 'Successful Attacks' not in df_country_attacks.columns:
        print("错误：Excel 文件中缺少 'Country' 或 'Successful Attacks' 列。")
        return
    ...
    merged['Successful Attacks'] = merged['Successful Attacks'].fillna(0)

    # 使用对数刻度，以更好地展示数据分布，避免极值影响
    merged['log_attack_count'] = np.log1p(merged['Successful Attacks'])  # log1p: log(1 + x)
    ...
    # 设置图标题
    ax.set_title('Distribution of Successful Attacks', fontname='Times New Roman', fontsize=16, pad=20)
    ...
    cbar.set_label('Log(Successful Attacks + 1)', fontsize=12, fontname='Times New Roman')
...
if __name__ == '__main__':
    # 定义文件路径
    excel_file = "./result/excel/successful_countries.xlsx"
    shapefile = "./ne_110m_admin_0_countries/ne_110m_admin_0_countries.shp"
    output_image = "./result/image/Crime_Successful_distribution.pdf"
