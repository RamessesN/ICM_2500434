...
def generate_heatmap(excel_path, shapefile_path, output_path):
    ...
    # 检查必要的列是否存在
    if 'Country' not in df_country_attacks.columns or 'Attacks' not in df_country_attacks.columns:
        print("错误：Excel 文件中缺少 'Country' 或 'Attacks' 列。")
        return
    ...
    merged['log_attack_count'] = np.log1p(merged['Attacks'])  # log1p: log(1 + x)
    ...
    # 设置图标题
    ax.set_title('Distribution of near miss Attacks', fontname='Times New Roman', fontsize=16, pad=20)
    ...
    cbar.set_label('Log(Attacks + 1)', fontsize=12, fontname='Times New Roman')
...
if __name__ == '__main__':
    # 定义文件路径
    excel_file = "./result/excel/near_miss_countries.xlsx"
    shapefile = "./ne_110m_admin_0_countries/ne_110m_admin_0_countries.shp"
    output_image = "./result/image/Crime_NearMiss_distribution.pdf"
