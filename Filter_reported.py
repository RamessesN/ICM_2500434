...
def save_successful_countries(countries_count, output_file):
    ...
    print(f"Near miss countries saved: {output_file}")
...
def main():
    ...
    nearMiss_stats_excel = "./result/excel/near_miss_countries.xlsx"  # 输出 Excel 文件路径

    # 确保输出目录存在
    os.makedirs(os.path.dirname(nearMiss_stats_excel), exist_ok=True)

    # 处理 JSON 文件，获取成功攻击的国家及其次数
    nearMiss_countries_count = process_json_files(json_directory)

    # 保存结果到 Excel 文件
    save_successful_countries(nearMiss_countries_count, nearMiss_stats_excel)
