...
def extract_successful_countries(json_data):
    ...
    if security_incident != "False positive":
        return []
    ...
    for filename in os.listdir(json_dir):
        if filename.endswith(".json"):
            ...
            for code in country_codes:
                if code != 'Unknown':
                    if code in nearMiss_countries_count:
                        nearMiss_countries_count[code] += 1
                    else:
                        nearMiss_countries_count[code] = 1

    return nearMiss_countries_count
...
def save_successful_countries(countries_count, output_file):
    ...
    if not countries_count:
        print(f"No false positive attacks found. {output_file} not created.")
        return
    # 构建包含国家代码、国家全名和成功攻击次数的列表
    data = []
    for code, count in countries_count.items():
        name = get_full_country_name(code)
        data.append({'Country': name, 'Attacks': count})
    df = pd.DataFrame(data)
    df = df.sort_values('Attacks', ascending=False)
    df.to_excel(output_file, index=False, engine='openpyxl')
    print(f"False positive countries saved: {output_file}")
...
def main():
    ...
    nearMiss_stats_excel = "./result/excel/false_positive_countries.xlsx"  # 输出 Excel 文件路径

    # 确保输出目录存在
    os.makedirs(os.path.dirname(nearMiss_stats_excel), exist_ok=True)

    # 处理 JSON 文件，获取成功攻击的国家及其次数
    falsePositive_countries_count = process_json_files(json_directory)

    # 保存结果到 Excel 文件
    save_successful_countries(falsePositive_countries_count, nearMiss_stats_excel)
