...
def process_education_data():
    df_edu = pd.read_excel('DATA/API_SE.TER.ENRR_DS2_zh_excel_v2_454.xls', header=None)
    df_edu.columns = ['Country Code'] + years
    ...
    df_edu_long = pd.melt(
        ...
        value_vars=years,
        var_name='Year',
        value_name='Tertiary Education (%)'
    ).query(f'{analysis_years[0]} <= Year <= {analysis_years[1]}')

    # Clean and interpolate
    df_edu_cleaned = (
        df_edu_long.groupby('Country Code')
        .filter(lambda x: x['Tertiary Education (%)'].isnull().mean() <= missing_threshold)
        .assign(
            Year=lambda x: x['Year'].astype(int),
            Education=lambda x: x.groupby('Country Code')['Tertiary Education (%)']
            .transform(lambda s: s.astype(float).interpolate(method='linear'))
        )
        .dropna(subset=['Tertiary Education (%)'])
    )
    return df_edu_cleaned
...
def analyze_data(df_edu, df_crime):
    # Merge datasets
    df_merged = pd.merge(
        df_edu,
        df_crime,
    ...
    try:
        ...
        rho, p = spearmanr(df_merged['Education'], df_merged['Cyber_Crime_Count'])
...
def visualize_data(df, rho, p):
    ...
    # 1. Education vs Crime Scatter Plot
    plt.subplot(131)
    sns.regplot(
        data=df,
        x='Education',
    ...
    plt.title(f'Tertiary Education vs Cyber Crime (ρ={rho:.2f}, p={p:.2e})', fontsize=12)
    ...
    plt.title('Tertiary Education Distribution', fontsize=12)
    plt.xlabel('Tertiary Education Enrollment (%)')

    # Save and display
    plt.tight_layout()
    plt.savefig(
        f'{output_dir}/education_crime_analysis_{timestamp}.png',
        dpi=300,
        bbox_inches='tight',
        facecolor='white',
        edgecolor='none'
...
if __name__ == "__main__":
    df_edu = process_education_data()
    df_crime = process_crime_data()
    df_merged, rho, p = analyze_data(df_edu, df_crime)
